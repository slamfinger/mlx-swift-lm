...
public struct GenerateCompletionInfo: Sendable {
    ...
    /// Which physical cache path the prompt took, attributed by a cache-owning
    /// session: `extend`, `extend-main`, `exact-n1`, `rewind`,
    /// `fork-no-rewind`, `rebuild`, or `cold`. `nil` when the caller did not
    /// attribute a reuse decision.
    public internal(set) var cacheReuseMode: String? = nil

    /// When the turn rebuilt despite a warm ledger (`fork-no-rewind`), the
    /// number of leading prompt tokens still identical to the ledger — the
    /// divergence point, in prompt-token space. `nil` for every other mode.
    public internal(set) var cacheForkCommonTokens: Int? = nil

    /// The ledger length the fork index is measured against, so callers can
    /// report `common/ledger` without re-deriving the denominator. `nil`
    /// unless ``cacheForkCommonTokens`` is set.
    public internal(set) var cacheForkLedgerTokens: Int? = nil
...
public enum Generation: Sendable {
    ...
    /// ``GenerateCompletionInfo/cacheReuseMode``, and
    /// ``GenerateCompletionInfo/cacheForkCommonTokens``.
    func attributingCachedPromptTokens(
        _ count: Int,
        ...
        info.cacheReuseMode = reuseMode
        info.cacheForkCommonTokens = forkCommon
        info.cacheForkLedgerTokens = forkLedger
        return .info(info)
    }
}
